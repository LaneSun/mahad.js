...
export class MahadArray extends Array {
    ...
    exchange(src, src_end_or_tar, tar = undefined, tar_end = undefined) {
        ...
        [src_sidx, src_eidx, tar_sidx, tar_eidx] = [
            src_sidx,
            src_eidx,
            tar_sidx,
            tar_eidx,
    ...
    modify_before(value, delete_count, inserts) {
        return this.modify(
            this.indexOf(value) - delete_count,
            delete_count,
            inserts,
        );
    }
    ...
    delete_at(value, delete_count = 1) {
        ...
        return offset >= 0
            ? this.delete(this.indexOf(value), delete_count)
            : [];
    }
    prefix(...values) {
    ...
    bind_map(src, fn) {
        const data = [].guard(
            null,
            null,
            (links) => links.forEach(([m, id]) => m.unlisten(id)),
        );
        const handle_modify = (
            src,
            tar,
            data,
            [, offset, delete_count, inserts],
        ) => {
            ...
            tar.modify(
                ...
                inserts.map((v, i) => {
                    ...
                    const update = () => {
                        ...
                        tar.set(
                            index,
                            fn(v, index, _make_marker(insert_links, update)),
                        );
                    ...
                    const res = fn(
                        v,
                        index,
                        _make_marker(insert_links, update),
                    );
    ...
    bind_reduce(src, fn, init_value) {
        ...
        const update = () => {
            ...
            const mark = _make_marker(links, update);
            this.val = src.reduce(
                (pre, cur, index) => fn(pre, cur, index, mark),
                init_value,
            );
            data[0] = links;
        };
...
export class MahadObject extends Object {
    static unfold(init_value, fn) {
        const res = new this();
        while (true) {
            let v;
    ...
    set_to(tar, fns, data = [], reset = true) {
        ...
        if (reset) {
            const cmds = Object.entries(this).map((
                [key, value],
            ) => [MC_MODIFY, key, value]);
            for (const cmd of cmds) {
    ...
    unset_to(tar, reset = true) {
        if (reset) {
            const cmd_pairs = Object.entries(this).map((
                [key, value],
            ) => [[MC_MODIFY, key, undefined], value]);
            const modify_handle = this[data_to][MC_MODIFY].get(tar);
            if (modify_handle) {
                const [fn, data] = modify_handle;
        ...
        const handle_modify = (src, tar, data, [, key, value]) => {
            if (value !== undefined) {
                ...
                const update = () => {
                    ...
                    tar.modify(
                        key,
                        fn(value, key, _make_marker(links, update)),
                    );
    ...
    bind_reduce(src, fn, init_value) {
        ...
        const update = () => {
            ...
            const mark = _make_marker(links, update);
            this.val = src.reduce(
                (pre, cur, key) => fn(pre, cur, key, mark),
                init_value,
            );
            data[0] = links;
        };
